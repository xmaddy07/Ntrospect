...
const Lugala = ({navigation, route}) => {
  ...
  return (
    <>
      {indicator && <Loader />}
      {ShowComment && <CommentModal Data={data} close={parentMethod} />}
      {popupVisible && (
        <JournalInput
            ...
            <View
              ...
              {detail?.export?.map(item => {
                return (
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate('Exports', {Data: item, id: data,icons:true,product:true})
                    }>
                    <Image
