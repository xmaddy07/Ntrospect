...
const CommentForNews = ({loader, y, Data, close}) => {
  ...
  const View_Comment = async () => {
    NewsArticle_viewCmnt_APIs({
      ...
      .catch(err => {
        console.log('err in view-news', err);
        setLoadingCursor(false);
      });
  };
  ...
  const commentAPIData = async => {
    ...
    formdata.append('news_id', Data),
      formdata.append('comment', comment),
      NewsArticle_Cmnt_APIs(
        {url: 'news-create-comment', Auth: user.token},
        formdata,
      )
        .then(res => {
  ...
  const DisLikeApi = async txt => {
    const formdata = new FormData();
    formdata.append('news_id', Data),
      formdata.append('news_comment_id', txt),
      formdata.append('is_like', '0'),
      NewsArticle_cmntLiDis_APIs(
        {url: 'like-dislike-news-comment', Auth: user.token},
        formdata,
      )
        .then(res => {
          setcomntstatus(res);
